 ...
 */
package com.github.zabetak.calcite.tutorial.setup.com.github.zabetak.calcite.tutorial;

import org.apache.calcite.linq4j.Enumerable;
import org.apache.calcite.sql.parser.SqlParseException;

import com.github.zabetak.calcite.tutorial.EndToEndExampleLuceneSimple;
...
/**
 * Tests for {@link EndToEndExampleLuceneSimple}.
 */
public class EndToEndExampleLuceneSimpleTest {
  ...
  void testPredefinedTpchQueriesRun() throws Exception {
    Files.walkFileTree(Paths.get("queries", "tpch"), new SimpleFileVisitor<Path>() {
      @Override public FileVisitResult visitFile(final Path file, final BasicFileAttributes attrs) throws IOException {
        try {
          EndToEndExampleLuceneSimple.main(new String[]{file.toString()});
        } catch (Exception e) {
  ...
  void testFullScanTpchTablesRuns() throws SqlParseException {
    for (TpchTable t : TpchTable.values()) {
      EndToEndExampleLuceneSimple.execute("SELECT * FROM " + t.name());
    }
  }
  ...
  void testCountTpchTables() throws SqlParseException {
    ...
    for (TpchTable t : TpchTable.values()) {
      Enumerable<Object[]> result = EndToEndExampleLuceneSimple.execute("SELECT COUNT(1) FROM " + t.name());
      Long actualCnt = (Long) result.iterator().next()[0];
  ...
  void testFilterOnPKAllColumnsValueTypesMatch() throws SqlParseException {
    ...
    for (Map.Entry<String, Object[]> q2r : queryToRow.entrySet()) {
      ...
      assertArrayEquals(expectedRow, EndToEndExampleLuceneSimple.execute(query).single());
    }
  }
}
